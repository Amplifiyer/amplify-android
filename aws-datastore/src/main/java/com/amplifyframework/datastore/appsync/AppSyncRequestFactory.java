...
final class AppSyncRequestFactory {
    ...
    static <M extends Model> AppSyncGraphQLRequest<ModelWithMetadata<M>> buildDeletionRequest(
            ModelSchema modelSchema, String objectId, Integer version, QueryPredicate predicate)
            throws DataStoreException {
        try {
            Map<String, Object> inputMap = new HashMap<>();
            inputMap.put("id", objectId);
            inputMap.put("_version", version);
            return buildMutation(modelSchema, inputMap, predicate, MutationType.DELETE);
        } catch (AmplifyException amplifyException) {
            throw new DataStoreException("Failed to get schema for model.",
                    amplifyException, "Validate your model file.");
        }
    }
    ...
    static <M extends Model> AppSyncGraphQLRequest<ModelWithMetadata<M>> buildUpdateRequest(
            M model, ModelSchema modelSchema, Integer version, QueryPredicate predicate)
            throws DataStoreException {
        Map<String, Object> inputMap = new HashMap<>();
        try {
            inputMap.putAll(modelSchema.getMapOfFieldNameAndValues(model));
            inputMap.put("_version", version);
            return buildMutation(modelSchema, inputMap, predicate, MutationType.UPDATE);
